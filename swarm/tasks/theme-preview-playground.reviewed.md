...
Reviewed by code inspection and browser testing on multiple themes (Radical, Solarized Light, Cobalt2).

**Verified:**
- All 4 scenes render correctly with proper theme colors and fonts
- Tab switching works
- Full Width toggle present and functional
- Component correctly placed between Fonts and Export sections
- Google Fonts loading works with deduplication
- TypeScript compiles cleanly
- Props match the Convex schema types exactly
- Light and dark themes both render well
- Inline styles properly isolate the preview from page styles

**Fixed:**
- `useGoogleFonts` hook was missing a cleanup function — stale `<link>` elements would accumulate in `<head>` when navigating between theme pages. Added `return () => { ... }` cleanup to the useEffect.
