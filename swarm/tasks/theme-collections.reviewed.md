...
Reviewed by agent. Found and fixed the following issues:

1. **Duplicate auth session lookup in `get` query** — The `get` query resolved the current user's session twice (once for visibility check, once for `isOwner`). Consolidated into a single lookup at the top of the handler.

2. **Inefficient auth session resolution** — `get` and `listByUser` queries used `.query("authSessions").filter(q => q.eq(q.field("_id"), session)).first()` instead of the direct `ctx.db.get(session)`. Replaced with `ctx.db.get()` for both queries.

3. **Missing compound index for duplicate prevention** — `collectionItems` had no index on `(collectionId, themeId)`, so `addTheme`, `removeTheme`, and `listMyCollections` all used `.filter()` to match `themeId` after the `by_collection` index — violating Convex guidelines ("Do NOT use `filter` in queries"). Added `by_collection_and_theme` index to schema and updated all three functions to use it.

4. **Silent error handling in AddToCollectionModal** — Both `handleToggle` and `handleCreate` caught errors silently (`// error handled silently`). Users got no feedback when mutations failed. Added toast notifications for error cases.

All fixes verified: TypeScript compiles clean, Vite build succeeds.
