...
Reviewed by Claude on 2026-03-28. All acceptance criteria met. Browser testing confirmed:
- Theme detail page renders correctly (color palette, live preview, fonts, comments)
- Edit/delete buttons correctly hidden for non-owners
- Star button properly disabled for unauthenticated users
- Full page layout is clean and consistent

### Bugs found and fixed

1. **Critical: Slug mismatch after rename** — The client-side `slugify()` function used a different algorithm than the server-side `generateSlug()` (different regex patterns for special characters). Additionally, the server's `uniqueSlug()` may append a numeric suffix for collisions, which the client couldn't predict. After renaming a theme, the client would redirect to a potentially wrong URL. **Fix:** Made `themes.update` mutation return `{ slug }` so the client uses the authoritative server-generated slug for redirect. Removed the now-unused client-side `slugify()` function.

2. **Bug: Self-collision in `uniqueSlug` during update** — When updating a theme's name, `uniqueSlug()` would find the *current theme's own slug* as a collision and unnecessarily append `-2`. For example, changing only the description while also sending the same name would generate a different slug. **Fix:** Added an optional `excludeId` parameter to `uniqueSlug()` and passed the current theme's ID when calling it from `update`.
