...
function UserProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"themes" | "collections">("themes");
  const {
    results: themes,
    status,
    loadMore,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      {/* Tabs */}
      <div>
        <div className="flex gap-1 p-1 rounded-lg bg-td-secondary w-fit mb-6">
          <button
        ...
        {activeTab === "collections" && (
          <>
            {collections && collections.length > 0 ? (
              ...
              <p className="text-td-muted">No collections yet.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-40 rounded-2xl bg-td-secondary animate-pulse"
                  />
                ))}
              </div>
            )}
